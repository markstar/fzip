...
package deng.fzip
{
	...
	public class FZip extends EventDispatcher
	{
		...
		/**
		 *
		 * Determines whether a file is contained in the ZIP archive, by index.
		 *
		 * @param index The index of the file to check
		 * @return Whether there's a file at the specified index or not
		 */
		public function hasFileAt( index:uint ):Boolean
		{
			return filesList ? filesList[index] : false;
		}
		...
		/**
		 * Determines whether a file is contained in the ZIP archive, by filename.
		 *
		 * @param name The filename of the file to check
		 * @return Whether there's a file at the specified index or not
		 */
		public function hasFileByName( name:String ):Boolean
		{
			return filesDict[name] ? true : false;
		}
